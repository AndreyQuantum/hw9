...
class StudentRepository:
    ...
    def update_student(self, student_id: int ,student_data: dict, session: Session) -> Student:
    ...
    def get_student_by_id(self, session: Session, student_id: int) -> Any | None:
        return session.scalar(select(Student).where(Student.id == student_id))

    def delete_student(self, student_id: int, session: Session) -> Student:
        student = self.get_student_by_id(session, student_id)
